...
为什么同样是 Claude 或 GPT，在 ChatGPT 里只能聊聊天，但在 Claude Code 里却能帮你写代码、跑测试、修 Bug？

答案就两个字：**Harness（框架）**。

LangChain 这篇博客把这个问题讲清楚了：模型只是大脑，Harness 才是让大脑干活的工具箱。
...
有意思的是，最好的 Harness 不一定是模型训练时用的那个。Terminal Bench 2.0 的排行榜显示，Opus 4.6 在 Claude Code 里得分远低于在其他 Harness 里的得分。优化 Harness 本身，还有很大空间。
...
随着模型越来越强，Harness 不会消失，只会演进。

就像 Prompt Engineering 到今天依然重要一样，Harness 工程也会持续有价值。原因很简单：好的环境配置、合适的工具、持久的存储、验证循环，这些让任何模型都更高效，无论基础智能多强。

## 结语

这篇文章的核心启发是：不要把 Agent 想成一个黑盒，它是模型加框架的组合。

模型负责智能，Harness 负责让智能变得有用。

如果你想构建自己的 Agent，可以从这个公式出发：先想清楚想要什么行为，再反推需要什么 Harness 组件。好的系统设计，能把 1 的智能放大成 10 的生产力。
