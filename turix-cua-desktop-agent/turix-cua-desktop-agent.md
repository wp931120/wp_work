...
graph TB
    subgraph 输入层
        User[用户指令] --> Config[配置文件]
    end

    subgraph 大脑层
        Brain[Brain 模型<br/>规划与决策]
        Planner[Planner 模型<br/>任务拆解]
    end

    subgraph 执行层
        Actor[Actor 模型<br/>屏幕理解与操作]
        Memory[Memory 模型<br/>记忆压缩与检索]
    end

    subgraph 输出层
        Action[屏幕操作<br/>点击/输入/滚动]
        Result[任务结果]
    end
...
sequenceDiagram
    ...
    participant Brain as Brain 模型
    participant Planner as Planner 模型
    participant Actor as Actor 模型
    participant System as 桌面系统

    User->>Brain: 发送任务指令
    Brain->>Planner: 请求任务拆解
    Planner->>Brain: 返回步骤规划
    loop 执行每个步骤
        Brain->>Actor: 发送操作指令
        Actor->>System: 截取屏幕
        Actor->>Actor: 理解屏幕并决策
        Actor->>System: 执行点击/输入操作
        Actor->>Brain: 汇报执行结果
    end
    Brain->>User: 任务完成
```
