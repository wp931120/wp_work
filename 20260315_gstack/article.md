...
> 这不是又一个 prompt 合集，而是一套能直接上手的"认知操作系统"

![](https://opengraph.githubassets.com/1679068800/garrytan/gstack)

**核心看点：**
- **4 天 1.2 万星**：GitHub 趋势榜第一，Fork 数破 1400
- **YC 总裁 Garry Tan 亲授**：把他管理 YC 投后公司的工程方法论开源了
...
先说一个反直觉的结论：**AI 编程工具最大的瓶颈，不是代码能力，而是"认知模式"单一。**

你让同一个模型同时做产品规划、技术评审、代码审查、发布上线、QA 测试——就像让一个既当 CEO 又当工程师又当测试的人，结果往往是"四不像"。
...
| `/review` | 偏执的资深工程师 | 找出那些能过 CI 但会在生产环境爆炸的 bug |
| `/ship` | 发布工程师 | 同步主干、跑测试、推分支、开 PR，一气呵成 |
...
Garry 在 README 里给了一个完整 demo，我强烈建议每个用 AI 编程的人都看看这个工作流。
...
这就是 Brian Chesky 模式：**不是实现显而易见的 ticket，而是从用户角度重新思考问题，找到那个不可避免、令人愉悦、甚至有点魔法的版本。**
...
gstack 最让我惊艳的不是 prompt 设计，而是**浏览器自动化的技术实现**。

### 传统 MCP 方案的问题

用 MCP（Model Context Protocol）做浏览器自动化，有三个硬伤：
...
在一个 20 次命令的浏览器会话中，MCP 工具要在协议框架上烧掉 3-4 万 tokens，而 gstack 是零。

### 核心创新：基于 ref 的元素选择

gstack 的浏览器不是用传统的 XPath 或 CSS 选择器，而是**基于可访问性树的 ref 系统**：
...
[Greptile](https://greptile.com) 是 YC 投资的公司，自动审查 PR，能抓出 race condition、安全问题那些能过 CI 但生产爆炸的 bug。
...
几乎所有功能我都从 plan mode 开始。描述想建什么，然后用 `/plan-ceo-review` 压力测试**是不是在做正确的事**。
