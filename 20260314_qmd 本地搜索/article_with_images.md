...
![QMD 项目封面](article_images/002_qmd_github.png)
...
你有没有遇到过这种情况：本地存了几百篇 Markdown 笔记、会议记录、技术文档，想找某个内容时只能用文件名硬搜？

说实话，这在以前要么得手动翻文件夹，要么得把文档上传到云端 RAG 服务——但后者意味着你的会议记录、内部文档可能泄露给第三方。

笔者最近发现了一个很有意思的项目：QMD。它由 Shopify CEO Tobias Lutke 开发，创建仅 3 个月就拿到了 15k+ stars。核心卖点就一个：**你的文档，就该在你的设备上搜索，不需要上传到任何云端。**

更关键的是，它不是简单的关键词匹配，而是上了三重搜索技术：BM25 全文检索、向量语义搜索、LLM 重排序。全部本地运行。
...
所谓 QMD（Query Markup Documents），其实就像一个**私人的本地搜索引擎**——专门用来索引和搜索你的 Markdown 文件。

它的工作流程是这样的：

![图1](article_images/diagram_1_mermaid.png)

**它解决了什么问题？**

传统的本地搜索工具（比如 grep、Spotlight）只能做关键词匹配，但你想找"项目时间线"，文档里写的是"Q4 规划"，就搜不到了。

而云端的 RAG 服务虽然能理解语义，但你的文档得上传到别人的服务器。

QMD 的思路是：为什么不能本地跑一个高质量的 RAG 引擎？

---

## 核心原理

QMD 的搜索管道分四步，笔者容啰嗦一下每一步的作用：
...
把两套搜索结果用 Reciprocal Rank Fusion 算法合并，兼顾精确匹配和语义相关。

**第四步：LLM 重排序**

用一个 0.6B 的 Reranker 模型对合并后的结果重新打分，把最相关的排到前面。

这里有的读者就会问了，这么多模型本地跑得动吗？好问题。QMD 用的是量化后的 GGUF 模型，最大的查询扩展模型也只有 1.1GB，普通笔记本完全能扛住。
...
这里要注意，`collection` 本质上是一个路径别名，方便后续搜索时指定范围。
...
笔者用 QMD 索引了自己的技术笔记（约 200 篇 Markdown 文件），测试了几个查询：
...
从结果来看，QMD 的语义理解确实强过传统关键词搜索，尤其是当你记不清原文用了什么词的时候。
...
笔者认为，QMD 的核心价值不在于技术创新，而在于**工程取舍**：
...
如果你的场景是**个人知识库**或**团队内部文档**，笔者建议用 QMD；如果是面向公网的公开文档，那云端 RAG 可能更合适（毕竟不用让用户自己跑模型）。
...
QMD 确实是个很务实的工具，它没有追求什么技术突破，而是把"本地文档搜索"这件小事做到了极致。

不得不感叹一句：有时候，**把现有技术组合好，比发明新技术更有价值**。

QMD 就像把 BM25、向量搜索、LLM 重排序这三样东西缝合成了一台精密仪器，每台零件都不是新的，但组装起来就很好用。

从项目热度来看，创建 3 个月拿到 15k+ stars，说明大家对"本地优先"的 RAG 方案是有真实需求的。

**笔者的判断**：如果你经常需要搜索本地文档、笔记、会议记录，QMD 值得试试。尤其是你在意隐私、不想把文档上传到云端的话，这可能是目前最好的选择。

最后送一句话：数据在你手上，模型在你机器上跑，这才是真正的**你的知识库**。
